...
export const options = {
  ...
  // Thresholds remain the same, we still expect high performance.
  thresholds: {
...
// --- NEW: Programmatically generate a large number of pool names ---
const TOTAL_POOLS = 64;
const POOLS = Array.from({ length: TOTAL_POOLS }, (_, i) => `pool-region-${i}`);
// This will create an array like:
// ['pool-region-0', 'pool-region-1', ..., 'pool-region-63']
// ---
...
export default function () {
  ...
  if (Math.random() < 0.95) {
    ...
    const workerId = `worker-wide-${__VU}-${__ITER}`; // Use a different prefix
